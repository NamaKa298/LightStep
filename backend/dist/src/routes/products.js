...
router.get("/", async (req, res) => {
    ...
    `;
        ...
        if (genders) {
            ...
            conditions.push(`g.name IN (${genderList
        ...
        if (brands) {
            ...
            conditions.push(`b.name IN (${brandList
                .map((_, i) => `$${params.length + i + 1}`)
                .join(",")})`);
        ...
        if (sizes) {
            const sizeList = Array.isArray(sizes)
                ? sizes.map(Number)
                : [Number(sizes)];
            conditions.push(`s.eu_size IN (${sizeList
                .map((_, i) => `$${params.length + i + 1}`)
                .join(",")})`);
        ...
        if (ground_types) {
            const groundTypeList = Array.isArray(ground_types)
                ? ground_types
                : [ground_types];
            conditions.push(`gt.name IN (${groundTypeList
                .map((_, i) => `$${params.length + i + 1}`)
                .join(",")})`);
        ...
        if (uses) {
            ...
            conditions.push(`u.name IN (${useList
                .map((_, i) => `$${params.length + i + 1}`)
                .join(",")})`);
            params.push(...useList);
        }
        if (stability) {
            ...
            conditions.push(`p.stability IN (${stabilityList
                .map((_, i) => `$${params.length + i + 1}`)
                .join(",")})`);
        ...
        if (colors) {
            ...
            conditions.push(`c.hex_code IN (${colorList
                .map((_, i) => `$${params.length + i + 1}`)
                .join(",")})`);
...
router.get("/filters", async (req, res) => {
    try {
        const [brands, sizes, ground_types, uses, colors, genders] = await Promise.all([
            db_1.pool.query("SELECT name FROM brands ORDER BY name"),
            db_1.pool.query("SELECT DISTINCT eu_size FROM sizes ORDER BY eu_size"),
            db_1.pool.query("SELECT name FROM ground_types ORDER BY name"),
            db_1.pool.query("SELECT name FROM uses ORDER BY name"),
            db_1.pool.query("SELECT name, hex_code FROM colors ORDER BY name"),
            db_1.pool.query("SELECT name FROM genders ORDER BY name"),
        ]);
        res.json({
